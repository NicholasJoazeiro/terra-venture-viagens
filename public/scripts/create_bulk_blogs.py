...
blog_html_path = os.path.join(public_dir, "blog.html")
...
<head>
    ...
    <title>{title} | Blog Terra Venture</title>
    <!-- Google Fonts -->
...
<body>
    ...
    <!-- Article Hero -->
    <section class="article-hero" style="background-image: url('{image}');">
    ...
    <!-- Footer Simples -->
    <footer class="footer"
        style="padding: 40px 0; background-color: var(--color-dark); color: white; text-align: center;">
        <div class="container">
            <h3 class="logo-text" style="font-family: var(--font-heading); font-size: 1.5rem; margin-bottom: 15px;">
                Terra Venture</h3>
            <p style="opacity: 0.6; font-size: 0.9rem;">&copy; 2026 Terra Venture Viagens. Todos os direitos reservados.
            </p>
        </div>
    </footer>
</body>
...
for b in blogs:
    path = os.path.join(public_dir, b["filename"])
...
# Insert the cards into blog.html right after <div class="blog-grid">
with open(blog_html_path, "r", encoding="utf-8") as f:
    blog_content = f.read()

target = '<div class="blog-grid">'
cards_str = "\n".join(card_htmls)
new_blog_content = blog_content.replace(target, target + "\n" + cards_str)

with open(blog_html_path, "w", encoding="utf-8") as f:
    f.write(new_blog_content)

print(f"Sucesso! Gerados {len(blogs)} novos blogs!")
